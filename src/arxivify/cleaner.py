from arxivify.file_utils import (
    build_relative_path,
    combine_paths,
    copy_files,
    create_temp_dir,
...
class Cleaner:
    def __init__(self, input_dir=None, output_dir=None, tex=None, command_options=None, verbose=False):
        # Save the arguments
    ...
    def clean(self):
        ...
        # Compile the TEX files with bibliography compiler to find the dependencies.
        ...
        # Remove the temporary expanded directory and project directory.
        remove_temp_dir(expanded_dir_obj)
        remove_temp_dir(project_dir_obj)

        # Remove all auxiliary files (.aux, .log, .out)
        files_to_delete = {".aux", ".log", ".out"}
        deleted_files = delete_files_with_extension(self.output_dir, files_to_delete)
        self.logger.info("Deleted aux files: {}".format(deleted_files))

        self.logger.info('Check the cleaned project at "{}"'.format(self.output_dir))
    ...
    def expand_files(self):
        ...
        new_dir_obj, new_dir = self.latex_runner.run_latexpand(self.input_dir, target_files)
    ...
    def copy_input_files_to_project(self, project_dir):
        # Log the start
        self.logger.info("Start copying files from input directory to temporary project")
    ...
    def compile_tex_to_find_dependencies(self, project_dir):
        ...
        for tex_file in self.tex_files:
            ...
            fls_deps = self.latex_runner.run_latex_compiler(project_dir, full_path)
    ...
    def compile_bib_to_find_dependencies(self, project_dir):
        ...
        for tex_file in self.tex_files:
            ...
            deps = self.latex_runner.run_bib_compiler(project_dir, full_path)
    ...
    def copy_expanded_files_to_output(self, project_deps, expanded_dir):
        ...
        copy_files(project_deps, expanded_dir, self.output_dir, skip_nonexistent=True)
    ...
    def remove_unnecessary_blank_lines(self):
        # Log the start
        self.logger.info("Start removing unnecessary blank lines in output directory")
    ...
    def _init_input_files(self):
        ...
        self.relative_input_paths = [build_relative_path(f, self.input_dir) for f in self.input_files]
    ...
    def _check_tex_files(self):
        ...
        for tex_file in self.tex_files:
            ...
            if not does_file_exist(full_path):
                raise ValueError(
                    ('TEX file "{}" does not exist in the input' + ' directory "{}"').format(tex_file, self.input_dir)
                )
